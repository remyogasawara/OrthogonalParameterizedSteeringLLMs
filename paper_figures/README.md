...
Requirements: Python 3.10+, numpy, pandas, matplotlib >= 3.7, PyMuPDF (`pip install pymupdf`, used to compose
the parameterized-steering diagram) and torch (only to read the activations pickle). No model, no GPU.
`make_figs.py` reads `../activations/3-1_Llama-2-7b-chat-hf_layer14_....pkl` from this repository; everything
else comes from `data/`. The `extract_*.py` scripts additionally import this repository's `src` package, so they
need the repository environment (`environment.yaml`) and the pickles described below.
...
| `butterfly_power_coord_iclr.pdf` | coordination x power activations, original vs orthogonalized | `../activations/3-1_...pkl` | `fig5` (uses `butterfly_repro.compute`) |
...
| `warmth_butterfly.npz` | `extract_warmth_butterfly.py <pkl>` | warmth + sycophancy activations pickle (`llama2_7_pos_neg_acts_warmth_sycophancy.pkl`, 800 positive and 800 negative activations per behaviour); the projections are computed by `butterfly_repro.compute` |
