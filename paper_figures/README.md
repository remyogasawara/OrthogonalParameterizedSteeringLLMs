...
| `multi_steer_power_coord_iclr.pdf` | Fig 5: alpha-iterative vs orthogonal parameterized, measured on coordination with power as beta in [-1, 1] (left) + parameterized order quad measured on power (right) | `alpha_beta_coord_power.csv`, `quad_power_order.csv` | `fig_multi_steer(left_csv="alpha_beta_coord_power.csv", ...)` |
| `multi_steer_power_left_iclr.pdf` | alternative Fig 5 with the left panel measured on power (coordination as beta) | `alpha_beta_power_coord.csv`, `quad_power_order.csv` | `fig_multi_steer(name=...)` |
...
|---|---|---|
| `alpha_beta_power_coord.csv` | `extract_alpha_beta.py <pkl>` | the power x coordination run of `../scripts/run_alpha_beta_pairs.sh` (measured on power, coordination as beta; all four methods; alpha in [-2, 2] step 0.25, beta in [-2, 2] step 0.5); the same run as the paper's multi-steering sensitivity and range tables. `alpha_lookup` = round(alpha, 1), the alpha the interval map actually applies; the plots use it. |
| `alpha_beta_coord_power.csv` | `extract_alpha_beta.py <pkl> alpha_beta_coord_power.csv` | the same pair's second run of `../scripts/run_alpha_beta_pairs.sh`, measured on coordination with power as beta (all four methods, same grids); matches the paper's range table row "coordination | power". |
